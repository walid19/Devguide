...
1. Earth frame velocity estimates of the vehicle and wind are used to calculate a wind relative velocity estimate. For multi-rotors an estimate can be formed using fusion of drag induced specific forces in the X and Y body frame. See the section on drag specific force observations for details.
2. The wind relative velocity is rotated into the body frame, and the dynamic pressure along each axis is calculated.
3. The dynamic pressure is multiplied by the positional error coefficient to calculate the amount of pressure error due to motion of the body. The error for flow along each axis is summed to calculate a total pressure error.
4. The pressure error is converted to a height error using the local values of air density and height and subtracted from the baro height before the height is used by the EKF

The amount of error correction is controlled using the following parameters:

* EKF2\_PCOEF\_XP - This is the ratio of static pressure error to dynamic pressure generated by a positive wind relative velocity along the X body axis. 
* EKF2\_PCOEF\_XN - This is the ratio of static pressure error to dynamic pressure generated by a negative wind relative velocity along the X body axis.
* EKF2\_PCOEF\_Y - This is the ratio of static pressure error to dynamic pressure generated by a wind relative velocity along the Y body axis.
* EKF2\_PCOEF\_Z- This is the ratio of static pressure error to dynamic pressure generated by a wind relative velocity along the Z body axis.

* EKF2\_ASPD\_MAX -  Limits the magnitude  of airspeed along each axis that will be used to correct for pressure errors. This should be set to the maximum flight speed of the vehicle.

For multi-rotors, the position error coefficients can be tuned by flying the vehicle along a single body axis at maximum speed for a minimum of 5 seconds in a height conrol mode followed by a full stop. If the vehicle descends when moving and then recovers, then the corresponding EKF2\_PCOEF parameter for that flight direction needs to be adjusted in negative increments of -0.1 until there is no noticeable loss of height. If height is gained when moving then the coefficient should be adjusted in  positive increments.

For fixed wing aircraft, the EKF2\_PCOEF\_XP parameter will need to be calculated analytically based on calculation of the position error coefficient obtained from comparing of GPS and baro height from flight logs.
...
* Body drag specific forces \(m/sec\*\*2\) : Refer to drag\_innov in [ekf2\_innovations](https://github.com/PX4/Firmware/blob/master/msg/ekf2_innovations.msg).
